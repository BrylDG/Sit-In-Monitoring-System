...
function getAllSitInHistory($conn, $idNo) {
    ...
    $sql = "SELECT * FROM SitInHistory WHERE idNo = '$idNo' ORDER BY date, logout, login DESC";
    $result = $conn->query($sql);
...
function getAllSitInCurentAdmin($conn) {
    $sql = "SELECT * FROM SitInHistory WHERE date = CURDATE() ORDER BY date, logout, login DESC";
    $result = $conn->query($sql);
...
function fetchStudents($conn) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<p><strong>Full Name:</strong> ' . htmlspecialchars($row['firstName'] . ' ' . $row['middleName'] . ' ' . $row['lastName']) . '</p>';
        ...
        echo '<div class="studentContainer"><p>No students found.</p></div>';
    }
    $stmt->close();
}
